...
@router.post("/", response_model=TerritoireRead)
def create(payload: TerritoireCreate, db: Session = Depends(get_db)) -> TerritoireRead:
    return create_territoire(db, payload)


@router.get("/", response_model=list[TerritoireRead])
def read_all(skip: int = Query(0, ge=0), limit: int = Query(100, gt=0), db: Session = Depends(get_db)) -> list[TerritoireRead]:
    return list_territoires(db, skip=skip, limit=limit)


@router.get("/{territoire_id}", response_model=TerritoireRead)
...
@router.put("/{territoire_id}", response_model=TerritoireRead)
